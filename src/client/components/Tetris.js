...
const Tetris = (props) => {
	...
	const connect = useCallback(
		async (
			...
			if (!mainSocket) {
				...
				//turns on deadUser socket, which listens for which user triggered a game over on their end
	...
	return (
		//Adding a tetris wrapper to the page which will assigns a role to it
		//so that when the player clicks it, they can then use the arrows to move the shapes
		<StyledTetrisWrapper
			role="button"
			tabIndex="0"
			onKeyDown={(e) =>
